...
if (!class_exists('ERE_Profile')) {
    ...
    class ERE_Profile
    {
        ...
        public function update_profile_ajax()
        {
            ...
            // Update user zip
            if (!empty($_POST['user_zip_code'])) {
                $user_zip_code = sanitize_text_field(wp_unslash($_POST['user_zip_code']));
                if ( 0 < strlen( trim( preg_replace( '/[\s\#0-9_\-\+\/\(\)\.]/', '', $user_zip_code ) ) ) ) {
                ...
                update_user_meta($user_id, ERE_METABOX_PREFIX . 'author_zip', $user_zip_code);
